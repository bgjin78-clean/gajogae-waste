...
MAIN_BEFORE_COUNT = 30
MAIN_PROCESS_COUNT = 25
MAIN_AFTER_COUNT = 30
...
review_services = {
    "changwon": "trash-cleaning", "gimhae": "home-waste", "jinju": "moving-waste", "yangsan": "business-waste",
    "geoje": "home-waste", "tongyeong": "trash-cleaning", "sacheon": "moving-waste", "miryang": "business-waste",
    "haman": "home-waste", "changnyeong": "trash-cleaning", "geochang": "business-waste", "hapcheon": "moving-waste",
    "hadong": "home-waste", "namhae": "business-waste", "sancheong": "trash-cleaning", "hamyang": "home-waste",
    "uiryeong": "moving-waste",
    "busan": "home-waste", "busan-haeundae": "moving-waste", "busan-busanjin": "business-waste",
    "busan-suyeong": "trash-cleaning", "busan-dongnae": "home-waste", "busan-namgu": "business-waste",
    "busan-saha": "moving-waste", "busan-geumjeong": "home-waste", "busan-gijang": "business-waste",
    "ulsan": "home-waste", "ulsan-namgu": "business-waste", "ulsan-junggu": "moving-waste",
    "ulsan-bukgu": "trash-cleaning", "ulsan-donggu": "home-waste", "ulsan-ulju": "business-waste",
}

review_service_label = {
    "home-waste": "가정폐기물처리",
    "moving-waste": "이사폐기물처리",
    "business-waste": "폐업폐기물처리",
    "trash-cleaning": "쓰레기집청소",
}
...
def img_num(slug, kind, count, offset=0):
    seed = sum(ord(c) for c in f"{slug}-{kind}") + offset
    return (seed % count) + 1


def main_img(slug, kind, offset=0):
    if kind == "process":
        return f"/images/main/process-{img_num(slug, kind, MAIN_PROCESS_COUNT, offset):02d}.jpg"
    return f"/images/main/{kind}-{img_num(slug, kind, MAIN_BEFORE_COUNT, offset):02d}.jpg"
...
def create_internal_links(current_slug):
    ...
    for r in regions:
        ...
        if r["group"] == "부산":
            groups["부산 지역 폐기물처리"].append(f'<a href="/regions/{r["slug"]}.html">{label}</a>')
        elif r["group"] == "경남":
            groups["경남 지역 폐기물처리"].append(f'<a href="/regions/{r["slug"]}.html">{label}</a>')
        elif r["group"] == "울산":
            groups["울산 지역 폐기물처리"].append(f'<a href="/regions/{r["slug"]}.html">{label}</a>')

    html = """
    <section class="section area" id="region">
...
def create_related_reviews(current_slug):
    items = []
    if current_slug in review_services:
        service = review_services[current_slug]
        items.append(
            f'<a href="/reviews/{current_slug}-{service}.html">이 지역 작업후기 보기</a>'
        )
    ...
    for r in regions:
        ...
        if slug == current_slug:
            continue
        if slug in review_services:
            service = review_services[slug]
            label = review_service_label[service]
            items.append(f'<a href="/reviews/{slug}-{service}.html">{r["name"]} {label} 후기</a>')
        if len(items) >= 8:
            break
...
def create_review_cards(name):
    return f"""
      <div class="review-card-grid">
        <div class="review-card">
          <div class="stars">★★★★★</div>
          <p>{name}에서 이사 후 남은 가구와 생활폐기물이 많았는데 비용 기준을 먼저 안내받아 편했습니다.</p>
          <strong>{name} 고객님</strong>
        </div>

        <div class="review-card">
          <div class="stars">★★★★★</div>
          <p>폐업 후 남은 집기류가 많았는데 현장 확인 후 빠르게 정리해주셨습니다.</p>
          <strong>폐업폐기물 상담 고객님</strong>
        </div>

        <div class="review-card">
          <div class="stars">★★★★★</div>
          <p>쓰레기집청소와 폐기물처리를 함께 상담할 수 있어서 좋았습니다.</p>
          <strong>쓰레기집청소 고객님</strong>
        </div>

        <div class="review-card">
          <div class="stars">★★★★★</div>
          <p>엘리베이터가 없는 현장이었는데 반출 동선까지 꼼꼼히 확인해주셨습니다.</p>
          <strong>폐기물처리 고객님</strong>
        </div>
      </div>
    """
...
def html_template(region):
    ...
    related_reviews = create_related_reviews(slug)

    before1 = main_img(slug, "before", 0)
    before2 = main_img(slug, "before", 11)
    process1 = main_img(slug, "process", 3)
    process2 = main_img(slug, "process", 15)
    after1 = main_img(slug, "after", 5)
    after2 = main_img(slug, "after", 18)
...
<body>
  ...
  <main>
    <section class="section intro">
      <h2>{h2_intro}</h2>
      <p>{intro}</p>
      <p>폐기물처리 비용은 <strong>1톤 트럭 1대 기준 25만원부터</strong>이며,</p>
      <p>폐기물의 양, 건물의 형태, 층수, 엘리베이터 유무, 작업 공간의 넓이에 따라 달라질 수 있습니다.</p>
    </section>

    <section class="section cards" id="services">
    ...
    <section class="section work-photo-section" id="photos">
      <p class="section-label">작업사진</p>
      <h2>{name} 폐기물처리 작업 현장</h2>
      <p class="section-desc">
        작업 전 상태, 분류 과정, 정리 후 공간 변화를 사진으로 확인하실 수 있습니다.
      </p>

      <div class="work-photo-grid">
        <figure>
          <img src="{before1}" alt="{name} 폐기물처리 작업 전 현장 사진" loading="lazy">
          <figcaption>작업 전 현장 확인</figcaption>
        </figure>
        <figure>
          <img src="{process1}" alt="{name} 폐기물 분류 작업 사진" loading="lazy">
          <figcaption>폐기물 분류 작업</figcaption>
        </figure>
        <figure>
          <img src="{after1}" alt="{name} 폐기물처리 작업 후 사진" loading="lazy">
          <figcaption>작업 후 정리 완료</figcaption>
        </figure>
        <figure>
          <img src="{before2}" alt="{name} 생활폐기물 작업 전 사진" loading="lazy">
          <figcaption>현장 상태 확인</figcaption>
        </figure>
        <figure>
          <img src="{process2}" alt="{name} 폐기물 반출 작업 사진" loading="lazy">
          <figcaption>폐기물 반출 진행</figcaption>
        </figure>
        <figure>
          <img src="{after2}" alt="{name} 폐기물 정리 완료 사진" loading="lazy">
    ...
    <section class="section review-section" id="reviews">
      <p class="section-label">고객후기</p>
      <h2>{name} 폐기물처리 고객후기</h2>
      {create_review_cards(name)}
    </section>
    ...
    <section class="section process" id="process">
      ...
      <p class="process-desc">상담부터 현장 확인, 폐기물 분류와 처리까지 단계별로 체계적으로 진행합니다.</p>
      <div class="process-grid">
        <div class="process-card">
          <div class="step-number">01</div><div class="step-icon">☎</div>
          <h3>상담 접수</h3><p>전화 또는 문자로 {name} 지역과 폐기물 종류를 확인합니다.</p>
        </div>
        <div class="process-card">
          <div class="step-number">02</div><div class="step-icon">▣</div>
          <h3>현장 확인</h3><p>사진, 주소, 건물 구조와 작업 환경을 확인합니다.</p>
        </div>
        <div class="process-card">
          <div class="step-number">03</div><div class="step-icon">₩</div>
          <h3>비용 안내</h3><p>폐기물 양, 층수, 엘리베이터 유무에 따라 비용을 안내합니다.</p>
        </div>
        <div class="process-card">
          <div class="step-number">04</div><div class="step-icon">✓</div>
          <h3>분류 정리</h3><p>폐기물을 종류별로 분류하고 반출 가능하도록 정리합니다.</p>
        </div>
        <div class="process-card">
          <div class="step-number">05</div><div class="step-icon">🚚</div>
          <h3>운반 처리</h3><p>정리된 폐기물을 안전하게 운반하고 처리합니다.</p>
        </div>
        <div class="process-card">
          <div class="step-number">06</div><div class="step-icon">◎</div>
          <h3>마무리 확인</h3><p>작업 후 현장을 확인하고 깔끔하게 마무리합니다.</p>
        </div>
      </div>
    </section>

    <section class="section seo-link-section">
      <p class="section-label">관련 작업후기</p>
      <h2>{name} 폐기물처리 작업후기와 내부링크</h2>
      <p>
        지역별 작업후기와 폐기물처리 안내 페이지를 함께 연결하여
        사용자가 필요한 정보를 더 쉽게 확인할 수 있도록 구성했습니다.
      </p>
      ...
      <div class="contact-layout">
        ...
        <form class="contact-form" id="contactForm">
          ...
          <div class="form-row full">
            ...
            <textarea id="message" name="raw_message" rows="5" placeholder="폐기물 양, 건물 층수, 엘리베이터 유무, 사진 전달 가능 여부 등을 적어주세요."></textarea>
          </div>
    ...
    <section class="section faq">
      ...
      <div class="faq-item">
        ...
        <p>가능합니다. 쓰레기 정리, 폐기물 반출, 현장 정돈까지 상황에 맞게 진행합니다.</p>
      </div>
      <div class="faq-item">
        <h3>{name} 폐업폐기물처리도 가능한가요?</h3>
        <p>가능합니다. 사무실, 식당, 매장 폐업 후 남은 집기류와 불필요한 물품 정리가 가능합니다.</p>
      </div>
      <div class="faq-item">
        <h3>상담은 어떻게 하나요?</h3>
        <p>전화 또는 문자로 지역, 폐기물 종류, 현장 사진을 보내주시면 빠르게 안내 가능합니다. 상담 문의는 {PHONE}로 연락 주시면 됩니다.</p>
      </div>
    </section>
...
def create_sitemap():
    ...
    core_urls = [f"{BASE_URL}/"]
    core_urls += [f"{BASE_URL}/regions/{r['slug']}.html" for r in regions]
...
def main():
    ...
    print(f"완료: 지역 페이지 {len(regions)}개 생성")
    print("완료: 메인 리뉴얼 스타일 반영")
    print("완료: 작업사진 / 고객후기 / 관련후기 / 상담폼 적용")
